...
> 当前仓库只存放业务、产品和项目管理文档。前后端代码仓库已经建立，M2 询价报价和 M3 批次溯源切片已完成，当前进入 M4 订单协同阶段。
...
13. 仓库保留 Docker Compose 配置；当前低内存电脑暂不安装或启动容器，更换环境后将 Docker/WSL 数据放到 E 盘并补做数据库、缓存、对象存储和邮件集成验证。
