...
V014  通知任务及发送尝试
V015  品种俄文草稿、复核状态、独立公开快照和乐观版本
后续迁移  通知、Outbox、任务锁、成本、导出审计和多批次拆分能力
```

Flyway 迁移一旦进入共享环境不得修改原文件，只能新增更高版本迁移。

V015 的 `catalog_variety_localization` 以品种 ID 为主键。`draft_content` 可反复编辑，`published_content` 只有管理员在另一内部账号复核通过后才替换；客户接口只读取后者。`draft_version` 支撑乐观并发控制，退回意见与复核人保留在当前状态中。本阶段不提供中文原文编辑；若将来允许修改中文关键字段，须新增源版本关联及俄文失效/重新审核机制，再满足第 19 节第 10 项测试要求。
