...
| 重量 | 核心计算统一换算为公斤，使用 `NUMERIC(18,3)`；保留客户输入单位 |
...
- `trade_order_allocation`：订单明细、供应批次和分配数量；一个明细可以拆分到多个同品种批次。
- `trade_inventory_reservation`：业务对象类型、业务对象 ID、批次、数量、锁定阶段、失效时间、确认/释放时间和原因。订单使用 `ORDER_ALLOCATION` 引用订单分配，样品使用 `SAMPLE_ITEM` 引用样品明细。
...
- `PENDING_CONFIRMATION`：客户接受报价后生成，默认 2 个工作小时内处理，不扣减可售库存。
...
`PENDING_LOCK` 至 `FULFILLING` 可因业务或合规问题进入 `SUSPENDED`；尚未完成的订单可进入 `CANCELLED`，但必须先按事务释放未履约库存。详细物流步骤不膨胀订单主状态，统一记录在订单事件中。
...
V008  样品、审核、物流和抵扣
V009  订单、明细、批次分配和库存占用
V010  路线、路线版本、订单快照和履约节点
V011  文件、文档版本、授权和访问日志
V012  通知、Outbox、任务锁、成本和导出审计
```
